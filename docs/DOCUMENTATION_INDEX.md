# 📚 Documentation Index

Quick reference for all documentation files in the project.

## 🚀 App Launch – Apple App Store (Canada)

- **`LAUNCH_CHECKLIST.md`** – **Start here.** Single step-by-step checklist to get My Tackle Box live on the Apple App Store (Canada only). In-App Purchases, app info, screenshots, RevenueCat, build, submit.
- **`APP_STORE_ASSETS_MYTACKLEBOX.md`** – Optional: extra app name, descriptions, keywords (CA$6.99 / CA$49.99, no lifetime).
- **`APP_STORE_CONNECT_API_KEY_GUIDE.md`** – How to generate App Store Connect API keys for RevenueCat
- **`APPLE_DEVELOPER_ACCOUNT_GUIDE.md`** – Apple Developer Program requirements
- **`DEPLOYMENT_GUIDE.md`** – Backend deployment guide

## 💰 Subscriptions & RevenueCat

- **`REVENUECAT_REACT_NATIVE_SETUP_COMPLETE.md`** – RevenueCat setup for React Native
- **`REVENUECAT_INTEGRATION_SUMMARY.md`** – Quick reference for RevenueCat integration

## 📸 Screenshots

- **`SCREENSHOT_GUIDE.md`** - Complete guide for taking app screenshots for stores

## 🔒 Security

- **`SECURITY_CHECKLIST.md`** - Security best practices checklist
- **`SECURITY_AUDIT_BEFORE_PUBLIC.md`** - Security audit before making repo public
- **`SUPABASE_SECURITY_PATCH_GUIDE.md`** - Guide for applying Supabase security patches

## ⚙️ Setup & Configuration

- **`SUPABASE_EMAIL_BRANDING.md`** – Branded signup confirmation & password reset emails (Supabase templates + GitHub Pages)
- **`SUPABASE_KEEP_ALIVE_SETUP.md`** – Keep-alive setup for Supabase (if needed)

## 📄 Legal Documents

- **`PRIVACY_POLICY.md`** - Privacy Policy content
- **`TERMS_OF_SERVICE.md`** - Terms of Service content
- **`LEGAL_DOCUMENTS_URLS.md`** - URLs where legal documents are hosted

## 🧪 Testing

- **`BETA_TESTING_GUIDE.md`** - Guide for beta testing
- **`SHARE_WITH_TESTERS.md`** - Instructions for sharing app with testers

## 📱 App-Specific Documentation

- **`FishingLureApp/README.md`** – Main app README
- **`FishingLureApp/DEPLOYMENT_CHECKLIST.md`** – iOS deployment checklist
- **`FishingLureApp/SUBSCRIPTION_SETUP.md`** – Subscription setup for app (RevenueCat products)
- **`FishingLureApp/SECURITY_AUDIT_REPORT.md`** – App security audit

## 📖 Main Documentation

- **`README.md`** - Project README (root)

---

## 🗑️ Cleaned Up

Removed 29 redundant/outdated files including:
- Multiple screenshot guide duplicates
- Completed setup guides
- Outdated status documents
- Temporary troubleshooting docs
- Redundant GitHub Pages setup files

---

**Need to find something?** Use your IDE's search or check the appropriate category above!

